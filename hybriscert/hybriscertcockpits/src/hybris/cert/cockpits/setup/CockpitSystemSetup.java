 ...
 */
package hybris.cert.cockpits.setup;
...
import hybris.cert.cockpits.constants.HybriscertCockpitsConstants;
 ...
 */
@SystemSetup(extension = HybriscertCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(HybriscertCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(HybriscertCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						HybriscertCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/hybriscertcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
